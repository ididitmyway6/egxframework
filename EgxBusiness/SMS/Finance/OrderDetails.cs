...
{
    public enum TransactionType { SALES_ORDER = -10, PURCHASE_ORDER = -9, SALES_RETURNED = -8, PURCHASE_RETURNED = -4, CANCEL_ORDER = -6 }
	...
	public class OrderDetails
	{
        ...
        {
            sm = new SystemMessage();
            Subjects product = new Subjects() { ID = PRODUCT_ID }.GetByID();
            this.PRODUCT_NAME = product.SUBJECT_NAME;
            ...
            this.NET = this.TOT- Convert.ToDecimal(((Convert.ToSingle(this.TOT)*product.DISCOUNT.GetValueOrDefault(0f))/100));
            this.ONO = ono;
            this.Insert();
            return sm;
        }
